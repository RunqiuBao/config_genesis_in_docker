...
[ -e /opt/lib/python3.9/site-packages/taichi ] && mv /opt/lib/python3.9/site-packages/taichi/ /opt/lib/python3.9/site-packages/__deprecated_taichi && mv /usr/local/lib/python3.9/dist-packages/taichi-1.7.3.dist-info /usr/local/lib/python3.9/dist-packages/__deprecated_taichi-1.7.3.dist-info
(cd ../libs/Genesis/ && pip3 install -force-reinstall -r requirements.txt -i https://pypi.org/simple && pip install --prefix=/usr/local dist/genesis_world-0.2.1-cp39-cp39-linux_x86_64.whl --no-deps) && echo -e "${GREEN}installation of genesis succeeded!${RESET_COLOR}"
